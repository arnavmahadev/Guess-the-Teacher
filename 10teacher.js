...
function displayQuestion() {
    ...
    if (currentQuestion < 10) {
        ...
        let currentQuestionData = silhouette[randomNum];
        displayImage(currentQuestionData.image);


        createRadioOptions(silhouette, randomNum);


        silhouette.splice(randomNum, 1);


        document.getElementById('continueButton').style.display = 'none';
        document.getElementById('message').textContent = '';
    } else {
        showMessage('You got ' + correctCount + ' out of 10 correct!');
        document.getElementById('continueButton').style.display = 'none';
...
function checkAnswer() {
    ...
    if (selectedValue) {
        document.getElementById('submitButton').style.display = 'none';
        document.querySelectorAll('input[name="silhouetteOptions"]').forEach(radio => {
            radio.disabled = true;
        });
...
function nextQuestion() {
    ...
    document.querySelectorAll('input[name="silhouetteOptions"]').forEach(radio => {      //got forEach from https://www.w3schools.com/jsref/jsref_foreach.asp
